 ...
 *
 *
 *Created on: Nov. 15, 2020 Updated on: Mar. 28, 2021
 *Created Author: ferrie
 *
 */


// Any program that uses the bTree class must include "bTree.h" to access the
// object templat for bNode and the corresponding function (method)
// definitions.
...
char **makeSortIndex(char arg); // Returns an array of pointers to
								// strings in sort order

void inOrder(bNode *root); // The recursive function that performs
						   // inorder traversal

void inReverseOrder(bNode *root); // Same as InOrder with order of
								  // recursion reversed
